...
import { User, Lock, FileText, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
const Profile = () => {
  ...
  const { data: allSubmissions = [], isLoading: isLoadingSubmissions } = useMySubmissions();
  ...
  const onPasswordSubmit = async (data: PasswordFormData) => {
    try {
      ...
      toast.error(error.message || t('profile.password_update_failed'));
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <Badge variant="default" className="bg-green-500"><CheckCircle2 className="h-3 w-3 mr-1" />{t('admin.approved')}</Badge>;
      case 'rejected':
              ...
              <TabsContent value="profile" className="space-y-6 mt-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t('profile.profile_information_desc')}</CardDescription>
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardContent>
                    <form onSubmit={handlePasswordSubmit(onPasswordSubmit)} className="space-y-4">
                      ...
                      <Button 
                        ...
                        className="w-full md:w-auto"
                      >
                        {updatePassword.isPending ? t('common.loading') : t('profile.update_password')}
                      </Button>
                    </form>
                  </CardContent>
                </Card>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm font-medium">{t('profile.total_submissions')}</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                {isLoadingSubmissions ? (
                  ...
                  <Card>
                    <CardContent className="py-8 text-center text-muted-foreground">
                      {statusFilter 
                        ? t('profile.no_submissions_status', { status: t(`admin.${statusFilter}`) })
                  ...
                  <div className="space-y-4">
                    {filteredSubmissions.map((submission) => (
                      <Card key={submission.id}>
                        ...
                        <CardContent>
                          <div className="space-y-2">
                            <p><strong>{t('mosque.name')}:</strong> {(submission.data as any).name || 'N/A'}</p>
                            <p><strong>{t('mosque.address')}:</strong> {(submission.data as any).address || 'N/A'}</p>
                            <p><strong>{t('mosque.state')}:</strong> {(submission.data as any).state || 'N/A'}</p>
                            ...
                            {submission.status === 'rejected' && submission.rejection_reason && (
                              <Alert variant="destructive" className="mt-4">
                                <AlertDescription>
                                  <strong>{t('admin.rejection_reason')}:</strong> {submission.rejection_reason}
                                </AlertDescription>
                              </Alert>
                            )}
